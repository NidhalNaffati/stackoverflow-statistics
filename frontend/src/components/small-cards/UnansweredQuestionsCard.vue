...
const fetchData = async () => {
  try {
    ...
    if (props.programmingLanguage === 'all') {
      const response = await axiosInstance.get('number-of-unanswered-questions')
      ...
      const response = await axiosInstance.get(
        'number-of-unanswered-questions/' + props.programmingLanguage
      )
      if (response.status === 200) {
        serverData.value = await response.data
      } else {
    ...
    <div class="card">
      <div class="card-body p-3">
        <div class="row">
          <div class="col-8">
            <div class="numbers">
              <p class="text-sm mb-0 text-capitalize font-weight-bold">Unanswered Questions</p>
